...
@section('title') Indo-European Lexicon: PIE Etymon and IE Reflexes @stop
...
<p class='center'>Nearby etymon: &nbsp;&nbsp; 
	@if ($etyma->prevEtyma())
		{{ HTML::link('lex/master/' . $etyma->prevEtyma()->old_id, 'previous', array('title' => 'previous etymon with reflexes' )) }}
	@else
		first
	@endif
	&nbsp; | &nbsp;
	@if ($etyma->nextEtyma())
		{{ HTML::link('lex/master/' . $etyma->nextEtyma()->old_id, 'next', array('title' => 'next etymon with reflexes' )) }}
	@else
		last
	@endif
</p>
