...
class PublicController extends Controller {
	...
	{		
		$data = array();
		$data['etymas'] = LexEtyma::with('cross_references')->withCount('reflexes')->get()->sortBy('order');
	...
	public function lex_semantic_category($cat_abbr)
	{
		...
		$data['alpha_cats'] = LexSemanticCategory::get()->sortBy('text');
		$data['fields'] = LexSemanticField::withCount('etymas')->where('semantic_category_id', '=', $cat_id)->get()->sortBy('number');
	...
	public function lex_semantic_field($field_abbr)
	{
        ...
        $data['field'] = LexSemanticField::with('etymas','semantic_category')
            ->whereRaw("abbr = ?", array($field_abbr))->get();
